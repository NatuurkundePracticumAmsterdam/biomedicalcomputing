...
Visual Studio Code is een gratis, open-source _code editor._ Het is heel populair en is beschikbaar voor Windows, macOS en Linux. Je kunt het gebruiken om code te schrijven, maar ook om de code uit te voeren en de resultaten te bekijken. Verder zit de editor vol snufjes die je helpen bij het schrijven van de code waardoor je minder hoeft te typen en minder fouten maakt.

Het is handig om de code die bij elkaar hoort in één map te plaatsen en die map te openen met Visual Studio Code. De editor weet dan wat er bij elkaar hoort en dat helpt bij het schrijven en uitvoeren van je code. We gaan daarom alle code voor deze cursus in één map plaatsen. Binnen die map mag je best submappen aanmaken, maar open met Visual Studio Code steeds de hoofdmap van de cursus. Noem, voor het gemak, de map _BiomedicalComputing._
...
!!! opdracht-basis "Map aanmaken voor de cursus"
    ...
    === "macOS"
        ...
        1. Open Finder.
        2. Ga via de menubalk naar **Ga -> Thuismap**.
        3. Ga via de menubalk naar **Archief -> Nieuwe map**.
        4. Type als naam `BiomedicalComputing` (zonder spatie).

    === "Windows"

        5. Open Verkenner.
        6. Klik op het :octicons-chevron-right-16:-symbool meteen rechts naast het :fontawesome-regular-home:-icoon.
...
!!! opdracht-basis "Hello World!"
    ...
    Klik in het centrale deel van Visual Studio Code: de editor. Hier kun je de code typen in het bestand {{file}}`hello.py` die je daarna uit kunt voeren. Type in:
...
!!! info "Open altijd de goede map"
    ...
    De map {{folder}}`BiomedicalComputing` is je projectmap. Hierin komt alle code te staan die we tijdens deze cursus gaan schrijven. Je mag, als je dat wilt, submappen aanmaken en je code organiseren. Bijvoorbeeld per sessie, of per onderwerp. _Maar:_ als je Visual Studio Code opnieuw opent, open dan _altijd_ de hoofdmap{{folder}}`BiomedicalComputing`, en _niet_ een submap.[^vscode-submap]
...
!!! opdracht-meer "Meerdere condities"
    ...
    1. Laat je script, als je tussen de 18 en 20 jaar oud bent, printen `You're still a teenager, but already an adult!` Controleer ook of de persoon jonger is dan 4 of ouder dan 68. In dat geval hoef je niet te werken en heb je lekker veel vrije tijd.

!!! opdracht-meer "F-strings"
    
    Misschien heb je gemerkt dat als je `#!py print("Hi", name, "!")` gebruikt, dat Python automatisch spaties invoegt tussen de verschillende stukken. Dus `Hi Alice !` in plaats van `HiAlice!`. Als je meer controle wilt over waar wel en niet spaties komen, kun je zogeheten _f-strings_ gebruiken. Een f-string is een string (`#!py "Hi"`) waar je de letter `f` vóór schrijft (`#!py f"Hi"`). Als je dat doet dan werkt dat hetzelfde als een gewone string, _behalve_ dat dingen die tussen accolades `{}` staan worden vervangen. Je kunt dan schrijven: `#!py print(f"Hi {name}!")` met als uitvoer `Hi Alice!`. De variabele die tussen haakjes stond is vervangen door de _waarde_ van die variabele. Hierbij heb je dus kunnen kiezen voor _wel_ een spatie tussen Hi en Alice, maar niet tussen Alice en het uitroepteken.

    1. Pas je script aan zodat je f-strings gebruikt, maar _alleen_ waar dat nodig of handig is.

## Versiebeheer

We hebben de map {{folder}}`BiomedicalComputing` nu niet op OneDrive, iCloud, Dropbox, Google Drive of wat dan ook staan. We hebben dus geen cloud-opslag en géén backup. Dat is niet zo mooi als je je laptop kwijtraakt of als je per ongeluk bestanden wist. Bij veel cloudaanbieders kun je een gewist bestand nog 30 dagen terughalen. OneDrive bewaart zelfs (tijdelijk) oude versies van een bestand, maar daar hebben wij dus nu niets aan. We gaan dit oplossen, op een manier die veel gebruikt wordt door programmeurs: met _versiebeheer_.
...
!!! opdracht-basis "Van een bestaande map een repository maken"
    ...
    <div class="grid-tree" markdown>
        ...
        **Zorg dat Visual Studio Code is afgesloten en geen bestanden meer open heeft staan.** Je gaat een repository maken van een bestaande map: {{folder}}`BiomedicalComputing`.
        ...
        <br>
        <br>
        Open de repository in Visual Studio Code door op de knop `Open in Visual Studio Code` te klikken _of_ via het dropdown-menu: **Menu > Repository > Open in Visual Studio Code**. Probeer maar eens of je het {{file}}`name_and_age.py` script nog steeds kunt runnen.
    </div>
        <div>
        {{folder}}`BiomedicalComputing`  
        {{T}} {{file}} `hello.py`  
        {{L}} {{file}} `name_and_age.py`   
        </div>
    </div>  
...
???+ meer-leren "Openbare code en samenwerken"
    ...
    !!! opdracht-meer "Tailor"
        ...
        Als je nog nooit op GitHub bent geweest dan kunnen de pagina's nogal intimiderend overkomen. De informatiedichtheid is nogal hoog. Na een paar bezoeken weet je meestal wel waar je dingen kunt vinden. <br><br> David heeft een data-analyse app geschreven dat Tailor heet. Deze app wordt gebruikt bij natuurkundepractica voor studenten Medische natuurwetenschappen en Science, business and innovation. Interessant om eens te kijken wat je hierover kunt vinden op GitHub.
...
Je hebt al eerder, handmatig, in DNA gezocht naar een fout die is ontstaan tijdens DNA-replicatie. Voordat we dat proces programmeren, beginnen we met een kleinere stap: we schrijven een script dat de DNA-replicatie nabootst.

!!! info "Aansluiting MNW-programma"

    Bij het vak _Biochemie_ (jaar 1, periode 2) leer je onder andere over de structuur van DNA, hoe cellen DNA kopiëren en hoe fouten in dat kopieerproces worden hersteld. De DNA-opdracht loopt hier op vooruit. Zo krijg je alvast een idee van hoe programmeren ingezet kan worden bij vraagstukken die je later in de opleiding tegenkomt.

!!! opdracht-basis "Replicatie"
    
    Maak een nieuw bestand met de naam {{new_file}}`dna_replication.py.`

    1. We beginnen met een stukje `#!py strand1 = "ATATAGAGTC"`. Print de letters van de bijbehorende tweede streng (Engels: _strand_), volgens de regels van DNA-replicatie, los onder elkaar. Dus: voor iedere letter uit strand1, als de letter een A is, print een T, als de letter een C is, print een G, enz. Voeg (korte) comments toe om duidelijk te maken welk stukje code wat doet.
...
!!! opdracht-meer "Meer condities"
    Als je code geneste `#!py if`-statements bevat, zoals:
    ```py
    if ...:
        if ...:
            ...
    ```
    pas je code dan aan met `and` of `or` zodat je in totaal maar vier `#!py if`-statements nodig hebt (in plaats van acht).
...
[^vscode-submap]: Als je dat wel doet dan weet Visual Studio Code niet welke bestanden allemaal bij je project horen en draait hij de code niet in je projectmap, maar in de submap. Je code kan dan errors geven als die bestanden op een andere plek zoekt.
[^pro-account]: [https://github.com/education/students](https://github.com/education/students)
[^git]: GitHub is een gebruiksvriendelijke laag over Git, het eigenlijke versiebeheersysteem. Git wordt tegenwoordig door bijna iedereen gebruikt of ondersteund. Git is ontwikkeld door Linus Torvalds als alternatief voor het commerciële systeem dat gebruikt werd voor de ontwikkeling van de Linux kernel. Het begon als een zeer eenvoudig &mdash; en volkomen ongebruiksvriendelijk &mdash; programma. Later is het in een veel gebruiksvriendelijker jasje gestoken. Relatief dan: je moet nog steeds ingewikkelde commando's intypen om iets voor elkaar te krijgen.
