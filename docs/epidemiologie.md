...
Elke persoon bevindt zich op elk moment in precies één compartiment en kan alleen in de richting $S \rightarrow I \rightarrow R$ bewegen, nooit omgekeerd en nooit een stap overslaan.
...
Het model bestaat uit een stelsel vergelijkingen dat beschrijft hoe de grootte van elk compartiment verandert over de tijd. De snelheid waarmee personen van $S$ naar $I$ stromen hangt af van hoe vaak vatbare en besmette personen elkaar tegenkomen en hoe besmettelijk de ziekte is. De verandering van $S$ wordt gegeven door
\begin{equation}
\frac{dS}{dt} = - \beta \cdot \frac{S(t)}{N} \cdot I(t).
\end{equation}
De parameter $\beta$ geeft aan hoe besmettelijk de ziekte is. $I(t)$ geeft aan hoeveel besmette personen er op dat moment zijn. Hoe meer besmette personen, hoe groter de kans dat een vatbaar persoon in aanraking komt met de ziekte. De fractie $\frac{S(t)}{N}$ geeft de kans dat een willekeurig persoon een vatbaar persoon treft. De min in de differentiaalvergelijking geeft aan dat $S$ alleen maar kan afnemen. 

De stroom van $I$ naar $R$ is eenvoudiger:
\begin{equation}
\frac{dR}{dt} = \gamma \cdot I(t).
\end{equation}
Een vast aandeel besmette personen herstelt per tijdseenheid. De herstelsnelheid wordt aangegeven met $\gamma$.[^gamma] Aan het $R$ worden alleen maar personen toegevoegd.

[^gamma]: De herstelsnelheid $\gamma$ is te bepalen uit de gemiddelde besmettelijke periode. Als een persoon gemiddeld $d$ dagen besmettelijk is, geldt $\gamma = \frac{1}{d}$. $\beta$ is minder eenvoudig vast te stellen, omdat deze factor het gemiddeld aantal contacten per persoon per tijdseenheid en de kans dat een contact daadwerkelijk tot een besmetting leidt combineert. Beide grootheden zijn moeilijk rechtstreeks te meten, waardoor $\beta$ in de praktijk meestal wordt geschat op basis van uitbraakdata.

$I$ ontvangt personen vanuit $S$ en verliest personen naar $R$. De verandering van $I$ is het verschil tussen die twee stromen:
\begin{equation}
...
Tot nu toe hebben we het SIR-model beschreven met differentiaalvergelijkingen. Die beschrijven verandering op elk moment in de tijd. We hebben eerder gezien dat een numerieke aanpak goed werkt wanneer een analytische oplossing niet voorhanden is en dat is hier het geval. We berekenen elke tijdstap $\Delta t$ met 
\begin{equation}
S(t+\Delta t) = S(t) + \frac{dS}{dt} \cdot \Delta t
\end{equation}
en analoog voor $I$ en $R$. Hoe kleiner $\Delta t$, hoe nauwkeuriger de benadering.

<div id="opdr:eerste-dagen"></div>
!!! opdracht-basis "De eerste dagen van de uitbraak"

    Het is goed om de numerieke aanpak eerst op papier uit te rekenen voor een aantal tijdstappen. Zo begrijp je wat er per tijdstap gebeurt en kun je het voorbeeld later gebruiken om je programma te testen.
    ...
    Werk drie tijdstappen uit op papier. Maak een tabel met daarin $t$, $S$, $I$, $R$ en $N$. Begin bij $t=0$. Gebruik voor het berekenen van $S$, $I$ en $R$ de uitkomst van vergelijking 1 t/m vergelijking 3. Controleer ook of $N$ niet verandert tijdens de verschillende tijdstappen.

    Goed om te weten: het model behandelt de populatie als continu, ondanks dat een persoon in werkelijkheid niet deelbaar is. De waarden voor $S$, $I$ en $R$ kunnen daarom kommagetallen zijn. Rond voor deze opdracht de waarden af op één decimaal.

Nu je de eerste drie dagen met de hand hebt doorgerekend en je weet wat je voor elke tijdstap moet berekenen, gaan we het model programmeren. We bouwen de code stap voor stap op.

!!! opdracht-basis "Vijftig dagen later"

    1. Maak een nieuw bestand aan met de naam {{new_file}}`sir_model.py`. Definieer bovenaan in het bestand alle beginwaarden en parameters. Gebruik dezelfde beginwaarden en parameters als in de [_opdracht De eerste dagen van de uitbraak_](#opdr:eerste-stappen).
    2. Schrijf code die de waarden van $S$ en $I$ berekent voor 50 dagen. Bereken voor elke tijdstap ook $N$, zodat je zeker weet dat de populatie constant blijft. Je hoeft de getallen niet af te ronden. Zorg dat voor elke tijdstap het volgende geprint wordt:
        ```
        t = 0: S = 1990, I = 10, N = 2000
        t = 1: S = ..., I = ..., N = ...
        ```
    Zijn de getallen logisch? Neemt $S$ af en $I$ toe? Blijft $N$ constant? Commit.
    3. Voeg nu $R$ toe aan de code. Zorg dat voor elke tijdstap het volgende geprint wordt:
        ```
...
!!! opdracht-meer "Waarden afgerond weergeven"
    ...
    In de [_opdracht De eerste dagen van de uitbraak_](#opdr:eerste-dagen) heb je de waarden afgerond op één decimaal. Je programma doet dat nu niet. Met een f-string kun je getallen afgerond weergeven. De regel
        ```py
