...
- $S$ (_susceptible_, vatbaar): personen die vatbaar zijn voor de ziekte, zij zijn nog niet besmet;
...
Elke persoon bevindt zich op elk willekeurig tijdstip in precies één compartiment. Tijdens een uitbraak kan een persoon alleen overgaan van $S$ naar $I$ en vervolgens van $I$ naar $R$.
...
Voor dit model gelden een aantal aannames. De populatie is gesloten: er is geen geboorte, sterfte of migratie, waardoor de totale populatiegrootte $N = S + I + R$ constant blijft. Het model beschouwt alle personen als identiek: het maakt geen onderscheid tussen personen op basis van bijvoorbeeld leeftijd, gezondheid of gedrag. Verder gaat het model uit van homogene menging: iedereen heeft evenveel kans om contact te hebben met ieder ander. Het model houdt dus geen rekening met clusters zoals gezinnen of klassen. Tot slot geldt dat een persoon die besmet raakt direct besmettelijk is en na herstel direct en blijvend immuun is.
...
\end{equation}
Het minteken geeft aan dat het aantal vatbare personen door nieuwe besmettingen afneemt. De parameter $\beta$ bepaalt hoe makkelijk nieuwe besmettingen ontstaan door contacten tussen vatbare en besmette personen en geeft daarmee de besmettelijkheid van de ziekte weer. $I(t)$ geeft het aantal besmette personen op een bepaald tijdstip. De fractie $\frac{S(t)}{N}$ geeft aan welk deel van de populatie op tijdstip $t$ vatbaar is voor de ziekte. Omdat contacten willekeurig over de populatie plaatsvinden, is dit ook de kans dat een willekeurig contact met een vatbaar persoon plaatsvindt. Samen bepalen deze factoren hoe snel het aantal vatbare personen afneemt.
...
Het SIR-model is in onderstaand stroomdiagram[^stroomdiagram] visueel weergegeven.

![Compartimenten SIR-model](figures/flow_chart_sir.png){: style="width:75%"}

[^stroomdiagram]: Eduardo Fernandez Carrion, Ángel Ramos, Benjamin Ivorra en Beatriz López. "Mathematical models for introduction, spread and early detection of infectious diseases in veterinary epidemiology." (2017)

Tot nu toe hebben we het SIR-model beschreven met een stelsel differentiaalvergelijkingen. Die beschrijven de verandering op elk moment in de tijd. We hebben in de vorige sessie gezien dat een numerieke aanpak goed werkt wanneer een analytische oplossing niet voorhanden is en dat is hier het geval. We berekenen elke tijdstap $\Delta t$ met 
\begin{equation}
...
!!! opdracht-meer "Waarden afgerond weergeven"
    ...
    1. Zorg dat de waarden in de tabel worden weergegeven op één decimaal. Commit.
...
!!! opdracht-basis "Een functie toevoegen"
    ...
    1. Schrijf in het bestand {{file}}`sir_model.py` de functie `#!py simulate_sir()` met de volgende header: 
        ...
        ```
    De functie simuleert de uitbraak en print voor elke tijdstap de waarden van $S$, $I$, $R$ en $N$. Verplaats hiervoor de loop met de berekeningen en de printregels naar de functie. Bij een `#!py while`-loop verplaats je ook de regel die het startpunt van de loop instelt naar de functie. Zorg er daarbij voor dat deze regels code op de juiste manier ingesprongen zijn.
    2. Koppel de parameters uit de functieheader aan de juiste variabelen in de verplaatste code. Vervang waar nodig variabelenamen, bijvoorbeeld voor `beta`, `gamma` en `days`. Definieer daarnaast bovenaan in de functie de beginwaarden. Deze kun je koppelen aan `N`, `I0` en `R0`. Variabelen die ook bovenaan in het bestand zijn gedefinieerd, worden hiermee overbodig. Haal bovenaan in het bestand weg wat nu dubbelop staat.    
    3. Roep onderaan in het bestand de functie aan met de volgende waarden voor de parameters:
        ```py
...
!!! opdracht-basis "De uitbraak gevisualiseerd"
    ...
    4. Schrijf onder de functie `#!py simulate_sir()` de functie `#!py plot_sir()`. Deze functie ontvangt de lijsten met de tijd, $S$, $I$ en $R$. Schrijf in deze functie de code om $S$, $I$ en $R$ tegen de tijd weer te geven in dezelfde plot. Voeg duidelijke astitels, aslabels en een legenda toe.
    5. Roep onderaan in het bestand de functie `#!py plot_sir()` aan met de lijsten die je terugkrijgt uit de functie `#!py simulate_sir()`. Voer het bestand uit. Krijg je wat je verwacht? Commit.
